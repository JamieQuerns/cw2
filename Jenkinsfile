pipeline 
{
    ...
    stages
	{
		...
		stage("Stage 4 Deploying new image to Kubernetes") 
		{
				steps
				{
				  sh 'ssh -o StrictHostKeyChecking=no ubuntu@ec2-52-201-24-236.compute-1.amazonaws.com kubectl set image deployments.apps/nodeapp node-app-hjwmk=docker.io/jamiequerns/node_app:latest'
				}
		}
